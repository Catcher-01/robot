...
def _find_model():
    candidates = [
        # 安装包内 site-packages
        '/home/hyh/robot/hospital_ws/install/gesture_control/lib/python3.10/site-packages/gesture_control/models/hand_landmarker.task',
        # ROS share 目录
        '/home/hyh/robot/hospital_ws/install/gesture_control/share/gesture_control/hand_landmarker.task',
        # 源码 models 子目录
        '/home/hyh/robot/hospital_ws/src/gesture_control/models/hand_landmarker.task',
        # 源码根目录（setup.py 安装时）
        '/home/hyh/robot/hospital_ws/src/gesture_control/hand_landmarker.task',
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
    raise FileNotFoundError(f'找不到 hand_landmarker.task，候选路径: {candidates}')
...
class GestureMouseNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'加载手势模型: {model_path}')
